...
; 
;  Port to xa syntax and AUTORUN feature by Christopher Smith, 2025
;    Build with xa -C krusader.s
...
#define	BRKAS2		; if 1, BRK will assemble to two $00 bytes
...
#ifdef	INROM
;------------------------------------------------------------------------------
;  Handle the AUTORUN stuff.  You can set it yourself to an arbitrary address,
;    but these convenience macros will be useful for building standard-ish 
;    ROM images.
;------------------------------------------------------------------------------
#ifdef EXBASIC
#define AUTORUN $E000
...
S_RUN	LDA ARGS	
	...
	BNE S_ERR;SHLERR
S_OK	LDX #NUMCMD
S_NEXT	LDA CMDS-1,X	; find the typed command
	CMP ARGS
	BEQ GOTCMD
	DEX
	BNE S_NEXT
S_ERR	PHA		; put dummy data on the stack
	PHA
SHLERR	
	LDY #SYNTAX
...
PANIC
	...
	BNE P_SKIP
	LDA #$01
P_SKIP	STA (SRCSTL),Y	; Y is $00 from SHINIT
	RTS
...
LIST			; L 	- list all
	...
	JSR TOSTRT
	JSR GETARG
	BEQ L_NEXT	; no args, list from start
...
INSERT	
	...
	CLC
	BCC I_IN
I_ARGS	JSR GOTOLN	; if no such line will insert at end
I_IN	JSR INPUT	; Get one line
...
G_LOOP	LDA CURLNH+1,X
	...
	INC TEMP2
G_SKIP	LDX #$04
G_LOOP2	PLA		; get CURLN and LINEN from the stack
	STA LINEL-1,X
	DEX
	BNE G_LOOP2
	RTS
...
;	CPY #$00
	BEQ LCLERR
	STY YSAV
D_DOIT	JSR GOTOLN	; this leaves TEMP1 in Y and TEMP2 in X
	CPX #FAIL
	BEQ LCLERR
	LDA CURLNL
	STA MISC2L
	...
	BNE D_CHK	; If TEMP4 != TEMP2, we just need to check carry
	CPY TEMP1	; Compare low bytes when needed
D_CHK	BCC LCLERR	; If carry clear, 2nd argument is too low
D_INC	INY		; Now increment the second argument
	BNE D_CONT
	INX
D_CONT	STX TEMP2
	STY TEMP1
...
PRNTLN			; print out the current line (preserve X)
	...
	JSR PRLNNM
	LDX #$00
PL_PRINT	LDA LABEL,X
	BEQ PL_DONE	; null terminator
	JSR OUTCH
	INX
	;CPX #USESZ
	BNE PL_PRINT
...
NEXTCH			; Check for valid character in A
	...
	LDA #SP
#endif
NC_SKIP	CMP #SP		; valid ASCII range is $20 to $5D
...
CM_ERR	PLA
	...
	JMP SHWERR
CM_FDIR	DEX
CM_FOUND	TAY		; put mnemonic/directive code in Y
...
OF_NEXT	JSR NEXTCH	; catches ESC, CR and BS
	BCC OF_NEXT	; only allow legal keys
	JSR OUTCH	; echo
	STA IOBUF,X
	INX
	CMP #SP
...
OF_RET	RTS
OF_FILL	LDA TEMP1
	BEQ OF_NEXT	; just treat a space normally
...
INSSPC	
	...
	CMP #SP
	BEQ IS_JUMP
IS_GET	JSR NEXTCH	; handles BS, CR and ESC
	CMP #SP
	BNE IS_GET	; only let SP through
IS_JUMP	STA IOBUF,X	; insert the space
	INX
	JMP OUTCH
...
DETKN			; Load a line to the IOBUF
	...
	LDX #LABEL
DT_LBL	LDA (CURLNL),Y
	BEQ DT_EOP	; indicates end of program
	CMP #BLANK
	BNE DT_SKIP
	INY
	LDA #<DT_EOL
...
DT_LOOP	LDA (CURLNL),Y
	BEQ DT_EOL	; indicates end of line
	CMP #EOFLD
	BNE DT_CONT
	INY
...
ASSEM			; Run an assembly
	...
	JSR CRLF
	JSR MYPRPC
AS_NEXT	JSR DO1LN	; line is in the buffer - parse it
	;CPX #FAIL
	INX
	BEQ SHWERR
...
SE_NEXT	LDA ERRPRE-1,X
	...
	BEQ SE_SKIP
	CPY #MAXERR+1
	BCC SE_SHOW	; If error code valid, show req'd string
SE_UNKWN	LDY #UNKERR	; else show unknown error
 	BEQ SE_SKIP
#endif
SE_SHOW	CLC
	TXA		; sets A to zero
...
SE_SKIP	LDX #EMSGSZ	
#ifdef	UNK_ERR_CHECK
SE_LOOP	LDA ERRMSG,Y
#else
SE_LOOP	LDA ERRMSG-EMSGSZ,Y
#endif
	JSR OUTCH
	INY
	DEX
...
DO1LN	
	...
	BEQ DORTS
D1L_CONT	LDY #$00
D1L_LOOP	LDA (CURLNL),Y
	BEQ D1L_DONE
	INY
	BNE D1L_LOOP
D1L_DONE	INY		; one more to skip the null
D1L_ENDPR	;JMP UPDTCL
; falls through
...
MO_CHABY	CPY #$35
	...
	CLC
	ADC #ADJABY
MO_CONT	JSR DOBYTE	; we have the object code
#ifdef	BRKAS2
	CMP #$00
	BNE MO_MKARG
	JSR DOBYTE
#endif
MO_MKARG			; where appropriate, the arg value is in LVALL,H
...
P_NOMOD	CPX #EQU
	BEQ DOEQU
P_NOEQU	CMP #SP		; Is there a label?
	BEQ P_NOLABL	
...
DOMOD			; Do we have a new module?
	...
	CMP #SP
	BEQ DM_STORE
DM_SETPC	JSR ATOFR	; output finishing module end address (+1)
...
PT_LOOP	LDA (PTCHTL),Y	; copy symbol to COMM
	STA COMM,Y
	INY
	CPY #SYMSZ
	BNE PT_LOOP
	...
	BEQ PT_CONT	; need to copy its offset as well
	LDA TEMP1  
...
PT_DONE	PLA
	...
	STA CURPCL
PT_RET	RTS
PT_BRA	JSR ADDOFF	; BRA instructions have a 1 byte offset argument only
	CPX #FAIL
	BEQ PT_ERR
...
PT_ERR	LDY #$00
	JSR OUTSP
PT_LOOP2	LDA (PTCHTL),Y	; Show symbol that failed
	JSR OUTCH
	INY
	CPY #SYMSZ
	BNE PT_LOOP2
	DEY		; Since #UNKSYM = #SYMSZ - 1
	BNE PT_DONE	; always branches
...
AM_NOTSP	CPX #IMV	; immediate mode - '#'
	...
	LDA ARGS
	JMP DOREL
AM_DOIMM	CMP #$2C	; check exception first - STA
	BEQ BAD
	LDX #IMM
	...
	CPX #FAIL
	BEQ AM_RET
AM_IMMOK	STX CURADM	; handle immediate mode
	;LDX #01	; skip the '#'
	DEX		; X == IMM == 2
	JSR QTEVAL
	INX
	...
	;LDX #IMM
AM_RET	RTS
AM_NOTREL LDX #0		; check the more complicated modes
	LDA ARGS
...
CHKEXS			; Current addressing mode is in X
	...
	BNE CE_CONT	; but ABY is ok, so promote byte argument to word
	LDA CURMNE
	CMP #$28
	BCC CE_CONT
	CMP #$2F+1
	...
	BNE OK
CE_CONT	LDY #SPCNT	; check special includes
CE_LOOP	LDA SPINC1-1,Y	; load mnemonic code
	CMP CURMNE
	BNE CE_NEXT
...
DL_RET	RTS
DL_CONT	STA TEMP1	; store the value of the byte in TEMP1
	PLA
	CMP #PLUS
	BEQ DL_NONEG
	LDA TEMP1
...
EVAL			; Evaluate an argument expression
	...
	CMP #HIBYTE
	BNE EV_DOLBL
EV_HASOP	STA FRFLAG	; disables forward references when there
	INX		; is a '<' or a '>' in the expression
	LDA ARGS,X
EV_DOLBL	JSR CHKLBL	; is there a label?
	BCS EV_LBL	; yes - get its value
	JSR CONVRT	; convert the ASCII
	CPX #FAIL
	BEQ XERR	
	BNE XCONT	
EV_LBL	STX XSAV	; move X to Y
...
XCHKOP	LDY #$00
	...
	STA LVALL
EV_GETLO	STY LVALH	; keep LVALL, and zero LVALH
EV_STORE	LDA ARGS,X	; copy rest of args to COMM
	STA COMM,Y
	BEQ EV_DOVAL	
	CMP #SP
	BEQ EV_DOVAL
...
LB2VAL			; label to be evaluated is in ARGS + X (X = 0 or 1)
	LDY #$00
LV_NEXT	CPY #LBLSZ	; all chars done
	BEQ DOLVAL
	...
	INY		
	BNE LV_NEXT
LV_STOP	LDA #SP		; label is in COMM - ensure filled with spaces
LV_LOOP1	STA COMM,Y	; Y still points to next byte to process 
	INY
	CPY #LBLSZ
	BNE LV_LOOP1
DOLVAL	LDA #<COMM	; now get value for the label
	...
	LDA COMM
	CMP #DOT
	BEQ LV_LOCAL	; local symbol
	JSR SYMSCH
	BEQ LV_FREF	; if not there, handle as a forward reference
LV_FOUND	LDY #SYMSZ
	...
	STA LVALH
	RTS	
LV_LOCAL			; locals much the same
	LDX #$03	; select local table
	JSR SYMSCH
	BNE LV_FOUND	; if not there, handle as a forward reference
LV_FREF	LDA FRFLAG	; set when patching
	...
	LDX NFREF
	BEQ LV_CONT	; no symbols to skip
LV_LOOP2	CLC		
...
LV_SKIP	DEX
	BNE LV_LOOP2
LV_CONT	STA MISCL	; Reqd address is now in MISCL,H
...
STRSYM			; Store symbol - name at LABEL, value in MISC2L,H
	...
	CMP #DOT
	BNE STS_SRCH	; Starts with a dot, so local
	LDX #$03
STS_SRCH	JSR SYMSCH
...
STCONT	;LDA #LABEL
	...
	CPX #DOT
	BEQ STS_LSYM	; Starts with a dot, so local
	SEC		; Store symbol in global symbol table	
	LDA GSYMTL	; Make space for next symbol
	SBC #SYMSZ+2	; skip over existing symbols
	BCS STS_CONTG	; Reqd address is now in GSYMTL,H
STS_DWNHI	DEC GSYMTH	; carry bit clear - decrease high pointer
STS_CONTG	STA GSYMTL
...
STS_LSYM	LDA LSYMTH	; Store symbol in local symbol table	
	STA MISCH
	LDA LSYMTL	; Calculate storage address
	LDX NLSYM
	BEQ STS_CONTL	; no symbols to skip
STS_LOOP	CLC
...
STS_SKIP	DEX
	BNE STS_LOOP
STS_CONTL	STA MISCL	; Reqd address is now in MISCL,H
...
STORE	LDY #0		; First store the symbol string
	STY MISC2H
   	LDX #SYMSZ
STS_MV     LDA (MISC2L),Y 	; move bytes
        STA (MISCL),Y
        INY
        DEX
        BNE STS_MV
...
F2A_SKIP	PHA	
	...
	CMP #$23	; is it JMP?
	BNE F2A_NOJMP
F2A_JSR	;LDY #$2		; force 2 bytes for these two situations
	INY		; following code treats Y = 3 the same as Y = 2
F2A_NOJMP	PLA		; mode base back in A
...
F2A_1BYT	TAY		; mode index now in Y
F2A_CHECK	LDA ARGS,X
	BEQ F2A_DONE
	CMP #SP
	BNE F2A_CONT
F2A_DONE	LDA ARGS
	...
	BMI F2A_1		; 6502 has no INZ mode, so reduce
...
SR_CHMTCH	INY
	...
	BCS SR_CHK1	; will always branch
;SR_FAIL	LDX #FAIL	; X = $FF indicates failure
;SR_MATCH	RTS		; got it - index is in X, address is in A and TBLL,H
...
ASC2HX			; convert ASCII code in A to a HEX digit
	...
	AND #$0F   
AA_VALID	RTS
AA_ERR	LDA #FAIL	; this value can never be from a single digit, 
...
DSMBL	
	...
	;STA COUNT
DB_DSMBL2	JSR INSTDSP	; Disassemble and display instr.
	JSR PCADJ
	STA PCL		; Update PCL,H to next instr.
	STY PCH
	;DEC COUNT	; Done first 19 instrs
	;BNE DB_DSMBL2	; * Yes, loop.  Else DSMBL 20th
...
INSTDSP	JSR PRPC	; Print PCL,H
	...
	BCC ID_IEVEN
	ROR  		; * Test B1
	BCS ID_ERR	; XXXXXX11 instr invalid
	CMP #$A2	
	BEQ ID_ERR	; 10001001 instr invalid
	AND #$87	; Mask 3 bits for address mode
	;ORA #$80	; * add indexing offset
ID_IEVEN	LSR   		; * LSB into carry for
	TAX   		; Left/right test below
	LDA MODE,X	; Index into address mode table
	BCC ID_RTMODE	; If carry set use LSD for
	JSR LSR4
	;LSR   		; * print format index
	;LSR   		
	;LSR   		; If carry clear use MSD
	;LSR   
ID_RTMODE	AND #$0F	; Mask for 4-bit index
	BNE ID_GETFMT	; $0 for invalid opcodes
ID_ERR	LDY #$80	; Substitute $80 for invalid op,
...
ID_PPADR1	CPX #$03
	BEQ ID_PPADR5	; If X=3 then print address val
ID_PPADR2	ASL FORMAT	; Test next print format bit
	BCC ID_PPADR3	; If 0 don't print
	LDA CHAR1-1,X	; *  corresponding chars
	JSR OUTCH	; Output 1 or 2 chars
	LDA CHAR2-1,X	; *  (If char from char2 is 0,
	BEQ ID_PPADR3	; *   don't output it)
	JSR OUTCH
ID_PPADR3	DEX   
	BNE ID_PPADR1
...
ID_PPADR5	LDA FORMAT
	CMP #$E8	; Handle rel addressing mode
	LDA (PCL),Y	; Special print target adr
	BCC ID_PPADR4	; *  (not displacement)
ID_RELADR	JSR PCADJ3	; PCL,H + DISPL + 1 to A,Y
	TAX   
	INX   
	BNE PRNTYX	; *     +1 to X,Y
	INY   
...
PCADJ3	LDY PCH	
	...
	BPL ID_PCADJ4	; *  branch).  Extend neg
	DEY   		; *  by decrementing PCH
ID_PCADJ4	ADC PCL
	BCC ID_RTS		; PCL+LENGTH (or displ) + 1 to A
...
MSGSZ = $1B
MSG	.asc "NESSEW NEK YB 3.1 REDASURK",CR
...
XQINIT  LDA INITBL-1,X ;INIT XEQ AREA
        ...
        BEQ GC_XQ2       ;  WITH TRAILING NOPS
GC_XQ1    LDA (PCL),Y    ;CHANGE REL BRANCH
GC_XQ2    STA XQT,Y      ;  DISP TO 4 FOR
        DEY            ;  JMP TO BRANCH OR
        BPL GC_XQ1       ;  NBRANCH FROM XEQ.
...
;------------------------------------------------------------------------------
; In the patch-up of the original monitor, this stomps the handling of the 
; backspace, but Ken has rearranged things so that the prompt is printed first.
;  If we do this, we don't print prompts.
;------------------------------------------------------------------------------
#ifdef AUTORUN
ESCAPE		JMP	AUTORUN
...
NEXTCHAR        JSR 	GETCH1
		...
		CMP     #BSA1         ;  Backspace key?
                ...
                CMP     #ESC          ;  ESC?
#ifdef		AUTORUN
		BEQ	GET		; ... which is only most of the handler
#else
                BEQ     ESCAPE        ;  Yes
#endif
                INY                   ;  Advance text index
                BPL     NEXTCHAR      ;  Auto ESC if line longer than 127
WMNC_CONT			      ;  Line received, now let's parse it
...
NOTHEX          CPY     YSAVM         ;  Was at least 1 hex digit given?
                ...
                RTS
WM_CONT		BIT     MODEM         ;  Test MODE byte
